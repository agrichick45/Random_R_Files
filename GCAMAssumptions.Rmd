...
```{r}
SOC_GCAM_crops<- mask(cropRaster, cropMask, 
                           inverse = FALSE, updatevalue = NA, filename = "SOC_GCAM_crops.tif")
```
...
forestRaster<-rasterize(mapForest, LandUse, field="soilC", background=NA)
...
```{r}
SOC_GCAM_forests<- mask(forestRaster, forestMask, 
                           inverse = FALSE, updatevalue = NA, filename = "SOC_GCAM_forests.tif")
```
...
shrublandMask<- classify(LandUse, cbind(id=c(0, 1, 2, 3, 4, 5, 6, 7, 8), 
                                      v=c(NA, NA, NA, NA, 1, NA, NA, NA, NA)))

plot(shrublandMask)

shrubRaster<-rasterize(mapShrubland, LandUse, field="soilC", background=NA)
```

### Extract Land Use Values into a New Raster

```{r}
SOC_GCAM_shrubland<- mask(shrubRaster, shrublandMask, 
                           inverse = FALSE, updatevalue = NA, filename = "SOC_GCAM_shrubland.tif")
```
...
grasslandMask<- classify(LandUse, cbind(id=c(0, 1, 2, 3, 4, 5, 6, 7, 8), 
                                      v=c(NA, NA, NA, 1, NA, NA, NA, NA, NA)))

plot(grasslandMask)

grassRaster<-rasterize(mapGrassland, LandUse, field="soilC", background=NA)
```

### Extract Land Use Values into a New Raster

```{r}
SOC_GCAM_grassland<- mask(grassRaster, grasslandMask, 
                           inverse = FALSE, updatevalue = NA, filename = "SOC_GCAM_grassland.tif")
```
...
urbanMask<- classify(LandUse, cbind(id=c(0, 1, 2, 3, 4, 5, 6, 7, 8), 
                                      v=c(NA, NA, NA, NA, NA, 1, NA, NA, NA)))

plot(urbanMask)

urbanRaster<-rasterize(mapUrban, LandUse, field="soilC", background=NA)
```

### Extract Land Use Values into a New Raster

```{r}
SOC_GCAM_urban<- mask(urbanRaster, urbanMask, 
                           inverse = FALSE, updatevalue = NA, filename = "SOC_GCAM_urban.tif")
```
...
RIDMask<- classify(LandUse, cbind(id=c(0, 1, 2, 3, 4, 5, 6, 7, 8), 
                                      v=c(NA, NA, NA, NA, NA, NA, 1, NA, NA)))

plot(RIDMask)

RIDRaster<-rasterize(mapRID, LandUse, field="soilC", background=NA)
```

### Extract Land Use Values into a New Raster

```{r}
SOC_GCAM_RID<- mask(RIDRaster, RIDMask, 
                           inverse = FALSE, updatevalue = NA, filename = "SOC_GCAM_RID.tif")
```
...
tundraMask<- classify(LandUse, cbind(id=c(0, 1, 2, 3, 4, 5, 6, 7, 8), 
                                      v=c(NA, NA, NA, NA, NA, NA, NA, 1, NA)))

plot(tundraMask)

tundraRaster<-rasterize(mapTundra, LandUse, field="soilC", background=NA)
```

### Extract Land Use Values into a New Raster

```{r}
SOC_GCAM_Tundra<- mask(tundraRaster, tundraMask, 
                           inverse = FALSE, updatevalue = NA, filename = "SOC_GCAM_tundra.tif")
```
...
pastureMask<- classify(LandUse, cbind(id=c(0, 1, 2, 3, 4, 5, 6, 7, 8), 
                                      v=c(NA, NA, NA, NA, NA, NA, NA, NA, 1)))

plot(pastureMask)

pastureRaster<-rasterize(mapPasture, LandUse, field="soilC", background=NA)
```

### Extract Land Use Values into a New Raster

```{r}
SOC_GCAM_Pasture<- mask(pastureRaster, pastureMask, 
                           inverse = FALSE, updatevalue = NA, filename = "SOC_GCAM_pasture.tif")
```
...
grassTiff<-rast("SOC_GCAM_grassland.tif")
pastTiff<-rast("SOC_GCAM_pasture.tif")
...
tundraTiff<-rast("SOC_GCAM_tundra.tif")
urbanTiff<-rast("SOC_GCAM_urban.tif")
...
forestTiff<-rast("SOC_GCAM_forests.tif")
shrubTiff<-rast("SOC_GCAM_shrubland.tif")

mosaic3<-mosaic(forestTiff, shrubTiff, fun="mean", filename="mosaic3.tif", overwrite=TRUE, wopt=list())

tmpFiles(current=TRUE, remove=TRUE)

cropTiff<-rast("SOC_GCAM_crops.tif")
RIDTiff<-rast("SOC_GCAM_RID.tif")
...
GCAM_SOC_Est<-mosaic(semiMosaic1, semiMosaic2, fun='mean', filename="GCAM_SOC_Raster_Fixed.tif", overwrite=TRUE, wopt=list())
