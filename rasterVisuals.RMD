...
setwd("~/soil-carbon-potential-mandy/Datasets")

soilMoisture<-raster("GLDAS_soil_moisture/GLDAS_SM_statistics/processedMoisture_Raster.tif")
npp<-raster("MOD17A3_Science_NPP_mean_00_15/MOD17A3_Science_NPP_mean_00_15.tif")
npp[npp >= 60000] <- NA

temperature<-raster("WorldClim/wc2.1_30s_tavg_annual.tif")

temperature_cropped <- crop(x = temperature, y = npp)

r<-stack(soilMoisture, npp, temperature_cropped)
  
names(r) <- c("Soil","NPP", "Temperature")
 ...
 # Define the number of breaks
  ...
  col.matrixQ <- colmat(nbreaks = nBreaks, breakstyle = "quantile",
                       xlab = "Soil Moisture", ylab = "NPP", 
                       bottomright = "#F7900A", upperright = "#993A65",
                       bottomleft = "#44B360", upperleft = "#3A88B5",
                       saveLeg = FALSE, plotLeg = TRUE)
  
  bivmapQ <- bivariate.map(rasterx = r[["Soil"]], rastery = r[["NPP"]],
